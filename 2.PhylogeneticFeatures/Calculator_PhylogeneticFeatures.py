#nTargets = "A,B,C,D"
# 6 vocal learning birds
nTargets ="TAEGU,GEOFO,CORBR,MELUN,NESNO,CALAN"

# 15 waterbirds
#nTargets = "PELCR,EGRGA,NIPNI,PHACA,FULGL,PYGAD,APTFO,GAVST,PHALE,EURHE,CHAVO,BALRE,PHORU,PODCR,ANAPL"

# 6 birds of prey
#nTargets = "FALPE,CARCR,TYTAL,HALLE,HALAL,CATAU"

iRound = 8 # round for phylogenetic values 0.0000000X
## TARGET
...
###########################################################
#                                                         #
# calculator for genetic distance (Sum of Branch Lengths) #
# writer: Chul Lee                                        #
# data: 2016.10.27.                                       #
# version: 0.15                                           #
#                                                         #
# Warning #################################################
#                                                 #
#   Please keep this source code confidential     #
#                                                 #
###################################################
...
#nTree = "((((A:1,B:2):8,C:3):9,D:4):10,((E:5,F:6):11,G:7):12);"
...
def calculate_sum_branchlength(monoTarget_list, info_tree, sID_list):
    if len(monoTarget_list)==1:
        ...
        if iCNT_R_parenthesis < iCNT_L_parenthesis:
            ...
            SUM_OF_WHOLE_BL = round(SUM_OF_WHOLE_BL,iRound)
...
def scan_OriBranch(sTarget, unselected_target_list, target_list, info_tree, sID_list, nMarker_target):
    ...
    if iFlag == 0:
        ...
        print_line = "# "+ nMarker_target+" group: "+','.join(sTarget_list)+' / SumBL: '+str(round(iSum_OriginBranch,iRound))
        print(print_line)
...
def Run_SumOfBL(sID_list, info_tree, target_list):
    ...
    SUM_OF_TERMINAL_BL = round(SUM_OF_TERMINAL_BL,iRound)
    ...
    DistanceTN = round(DistanceTN,iRound)
    DistanceTB = round(DistanceTB,iRound)
    ...
    for monophyletic_target_list in target_clade_list:
        ...
        ProOriBL *= round(iLen_origin_branch,iRound)
        SumOriBL += round(iLen_origin_branch,iRound)
    ...
    for targetID in target_list:
        ...
        ProTerBL *= round(iLen_terminal_branch,iRound)
        SumTerBL += round(iLen_terminal_branch,iRound)
    ...
    print("# Distance between Termianl Nodes: " + str(DistanceTN))
    print("# Distance between Terminal Branches: " + str(DistanceTB))
    print("# Product of origin branch length: " + str(ProOriBL))
    print("# Sum of origin branch length: " + str(SumOriBL))
    print("# Product of terminal branch length: " + str(ProTerBL))
    print("# Sum of terminal branch length: " + str(SumTerBL))
    ...
    fpout.write("# Distance between Termianl Nodes: " + str(DistanceTN)+'\n')
    fpout.write("# Distance between Terminal Branches: " + str(DistanceTB)+'\n')
    fpout.write("# Product of origin branch length: " + str(ProOriBL)+'\n')
    fpout.write("# Sum of origin branch length: " + str(SumOriBL)+'\n')
    fpout.write("# Product of terminal branch length: " + str(ProTerBL)+'\n')
    fpout.write("# Sum of terminal branch length: " + str(SumTerBL)+'\n')
